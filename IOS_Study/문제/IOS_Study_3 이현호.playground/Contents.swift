...
let warrior: Character = Character(a: 1, b: 1)
...
//정답을 입력하시오.
extension Character{
    func move(moveTo:(a: Int, b: Int)){
        self.position.0 = self.position.0 + moveTo.a
        self.position.1 = self.position.1 + moveTo.b
    }
}

var warrior: Character = Character(a: 1, b: 1)

warrior.move(moveTo: pos)
...
//plusFunc(10) // 실행 결과 값 : 30
 ...
 /*
 plusFunc에 들어있는 plus 함수(클로저)는 외부 변수인 sum을 사용할 수 있는데, 이를 캡쳐라고 부른다.
 클로저의 캡쳐는 reference capture형식 이다.(중첩 함수 형식으로)
 plusClouser의 지역변수인 sum 은 stack 영역에 저장되어 있고, plus 클로저는 heap 영역에서 이를 참조한다.
 때문에 반복된 plus 함수의 호출에 sum의 값이 변경된다.
 */
